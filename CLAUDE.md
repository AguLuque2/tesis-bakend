...
3. **Validación con zod obligatoria** en todo endpoint que reciba body/query/params. Nunca confiar en el tipo que "debería" venir del frontend.
...
5. **Seguridad siempre presente**: la `service_role key` de Supabase nunca se usa desde el frontend ni se loguea. RLS es la última línea de defensa, no la única — el backend también valida permisos. Nunca loguear tokens, contraseñas ni datos personales sensibles.
6. **Sin código muerto ni comentado "por las dudas".** Si no se usa, se borra.
